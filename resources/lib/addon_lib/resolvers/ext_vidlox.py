...
class ExternalVidloxResolver(UrlResolver):
    name = "vidlox"
    domains = ['vidlox.tv']
    ...
    def get_media_url(self, host, media_id):
        ...
        try:
            sources = re.search(r'sources\s*:\s*\[(.+?)\]', html).groups()[0]
        except:
    ...
    @classmethod
    def _is_enabled(cls):
        return common.get_setting('VidloxResolver_enabled') == 'true'

    @classmethod
    def _get_priority(cls):
        try:
            return int(common.get_setting('VidloxResolver_priority')) - 1
        except:
            return 99
